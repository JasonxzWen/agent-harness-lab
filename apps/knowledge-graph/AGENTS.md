...
- 新增布局区域时，先判断它和现有导航、hero、画布、详情抽屉的关系，避免把主操作挤到屏幕外。
- 新增知识点时，必须让用户最快看到 `why / what / how`：为什么需要它、它是什么、它在本仓库如何实现。
- 每个知识点都要配一个 `why / what / how` 可视化元素。优先用 Mermaid 表达机制关系；需要页面内操作时，用 React/CSS 做轻量交互组件或动画，并提供 reduced-motion 兼容。
