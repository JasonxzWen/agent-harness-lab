...
- 默认使用中文，保留必要英文术语，例如 `Agent Loop`、`Tool Registry`。
- 语言要严谨、专业、短句、通俗。先说用户能看到什么、能做什么、为什么重要。
- 禁止使用空泛词和营销腔，例如“赋能、打造、沉淀、范式、生态、底座、抓手、心智、信息熵”。
- 不写“高大上”解释。能用一句话说明，就不要写成三句话。
- 页面文案优先服务快速阅读；文档文案优先服务快速执行。
- 新增展示文案时，必须能回答：这是什么？现在能做什么？下一步是什么？
...
- 每次改动完成后，都要用 Mermaid 给出验收顺序。能用时序图表达时，优先使用 `sequenceDiagram`。
- 所有改动都要附带全局思维检查：页面元素是否过多、是否影响其他元素展示、用户进入交互路径是否会疑惑、是否需要减少文案或合并区域。
- 所有组件改动都要检查交互易用性：按钮 hover、focus、active、点击后的反馈是否清楚；新增文本是否短、准、容易扫读；新增区域的排版是否让用户先看到重点动作。
- 新增知识点时，必须按 `why / what / how` 组织：为什么需要这个功能、它是什么、它如何在本仓库实现。
- 每个知识点都必须用可视化元素展示 `why / what / how`，可以是 Mermaid 图，也可以是 React/CSS 交互组件或轻量动画。
- 未实现的入口不能静默摆放；必须给出可见交互提示，明确显示“暂未实现”或同义短句。
- 用户偏好教学速度快 2-3 倍：少铺垫，快速进入机制、代码、验证和总结。
- 教学和站点文案都要去 AI 味，少用抽象名词，多用具体动作、文件、命令和结果。
- 每完成一个完整板块的学习，应该提交一次。
